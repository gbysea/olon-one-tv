...
const url = process.env.VITE_BoltDatabase_URL || '';
const key = process.env.VITE_BoltDatabase_ANON_KEY || '';

if (!url || !key) {
  console.error('Missing Bolt Database env variables: VITE_BoltDatabase_URL and VITE_BoltDatabase_ANON_KEY');
  process.exit(1);
}
